...
<td width="50%" align="left"><select name="category">
<option></option>
...
else if(isset($_POST['submit']) or isset($_GET['page']))
{
	...
	(!$_SESSION['category'])? $_SESSION['category']=$category : $category=$_SESSION['category'];
?>
<fieldset style="width:50%; margin-left:auto; margin-right:auto;">
<legend><?php echo $category; ?> mark details</legend>
...
//	$tbl_name="";		//your table name
	...
	*/
	$query = "SELECT COUNT(*) as num FROM studdetails WHERE category='$category'";
	$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages[num];
	if($total_pages==0)
	{
		echo "<script>alert('No students in the selected Category');location.href='markbycategory.php';</script>";
	}
	/* Setup vars for query. */
	$targetpage = "markbycategory.php"; 	//your file name  (the name of this file)
	...
	/* Get data. */
	$sql = "SELECT * FROM studdetails WHERE category='$category' ORDER BY std,division LIMIT $start, $limit";
...
name='mark2' style='background-color:inherit; color:inherit;'/></a></td>";
	...
	echo "<td><a href=printonestudent.php?ad=".$row->admno." style='text-decoration:none; color:inherit;'><input type='button' value='Print all' name='mark3' style='background-color:inherit; color:inherit;'/></a></td>";
}
else
{
...
<input name="back" type="button" value="<< Back" onClick="location.href='markbycategory.php'" style="float:right"/>
</fieldset>
